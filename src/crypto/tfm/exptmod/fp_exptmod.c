 ...
 */
static int _fp_exptmod(const fp_int * G, const fp_int * X, const fp_int * P, fp_int * Y)
{
  fp_int   M[64], res;
  fp_digit buf, mp;
...
int fp_exptmod(const fp_int * a, fp_int * b, const fp_int * c, fp_int * d)
{
   ...
   if (b->sign == FP_NEG) {
      ...
      fp_copy(a, &tmp);
      if ((err = fp_invmod(&tmp, c, &tmp)) != FP_OKAY) {
         return err;
      }
      b->sign = FP_ZPOS;
      err =  _fp_exptmod(&tmp, b, c, d);
      if (b != d) {
         b->sign = FP_NEG;
      }
      return err;
